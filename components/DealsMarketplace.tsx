...
export const DealsMarketplace: React.FC<DealsMarketplaceProps> = ({ selectedGovernorate }) => {
  ...
  useEffect(() => {
    const fetchDeals = async () => {
      ...
      try {
        ...
        setDeals(data.length > 0 ? data : mockData.deals(selectedGovernorate));
      } catch (error) {
        console.error('Error fetching deals:', error);
        setDeals(mockData.deals(selectedGovernorate));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <section className="py-16 relative" data-section="deals">
      {statusMsg && <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-secondary/20 border border-secondary/40 text-white px-4 py-2 rounded-xl">{statusMsg}</div>}
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {deals.length === 0 ? (
            <div className="col-span-full py-12 flex flex-col items-center justify-center text-center opacity-50"><Tag className="w-12 h-12 text-white/20 mb-4" /><p className="text-white/60 text-sm">{t('deals.noDeals') || 'No active deals at the moment.'}</p></div>
          ) : (
            deals.map((deal) => (
              <div key={deal.id} className="relative group backdrop-blur-xl bg-gradient-to-br from-accent/10 to-primary/10 border border-white/10 rounded-2xl p-6 hover:border-accent/50 hover:shadow-glow-accent transition-all duration-300 overflow-hidden text-start">
                <div className="absolute top-0 end-0 w-24 h-24"><div className="absolute top-0 end-0 w-full h-full bg-gradient-to-br from-accent to-primary transform rtl:-rotate-45 ltr:rotate-45 rtl:-translate-x-8 ltr:translate-x-8 -translate-y-8"></div><div className="absolute top-4 end-4 text-white font-bold text-lg transform rtl:-rotate-12 ltr:rotate-12">-{deal.discount}%</div></div>
                ...
                <div className="mb-4"><div className="flex justify-between text-xs text-white/60 mb-2"><span>{t('deals.claimed')}</span><span>{deal.claimed}/{deal.total}</span></div><div className="h-2 rounded-full bg-white/10 overflow-hidden"><div className="h-full bg-gradient-to-r from-accent to-primary transition-all duration-500" style={{ width: `${(deal.claimed / deal.total) * 100}%` }} /></div></div>
                <button onClick={() => { setStatusMsg(t('deals.claimedToast') || 'Deal claimed in prototype mode.'); setTimeout(() => setStatusMsg(null), 2500); }} className="w-full py-3 rounded-xl bg-gradient-to-r from-accent to-primary text-white font-semibold hover:shadow-glow-accent transition-all duration-200 flex items-center justify-center gap-2"><Tag className="w-4 h-4" />{t('deals.claimNow')}</button>
              </div>
            ))
          )}
        </div>
